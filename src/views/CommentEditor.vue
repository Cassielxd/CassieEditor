<template>
  <div class="flex w-full">
    <div class="grid  flex-grow card bg-base-300 rounded-box place-items-center">
      ...
      <BubbleMenu v-if="editor" :tippy-options="{ duration: 100, placement: 'bottom' }" :editor="editor"
        :should-show="({ editor }) => (isCommentModeOn && !editor.state.selection.empty && !activeCommentsInstance.uuid)"
        class="card bg-base-100 shadow-xl">
       
        <div class="card-body items-center text-center">
          <h2 class="card-title">评论</h2>
          <textarea class="textarea textarea-primary" v-model="commentText" cols="30" rows="4" placeholder="添加新的批注"
  ...
  setup() {
    ...
    let editor = useEditor({
      ...
      extensions: [
        ...
        Comment.configure({ isCommentModeOn: getIsCommentModeOn, HTMLAttributes: { class: "comment" } })
      ]
    });
